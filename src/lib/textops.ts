...
import { activeSource, baselineOffset } from './css'
...
export const retypePatch = (obj: TextObj, next: { size?: number; font?: FontKey }): Partial<TextObj> => {
  ...
  const source = activeSource(obj)
  const shift = baselineOffset(obj.font, obj.size, source) - baselineOffset(font, size, source)
